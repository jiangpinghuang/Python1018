...
class RecursiveAutoEncoder(nn.Module):
    ...
    def forward(self, children):
        parent, encode = self.encoder(children)
        decode = self.decoder(encode)
        return parent, encode, decode
...
def build_semcom(line, ssc_vec):
    ...
    tags = sents[2].split()
    label = torch.IntTensor([int(sents[0])])
    ...
    for i in range(args.epochs):
        ...
        for j in range(len(train_data)):
            ...
            for k in range(1, len(sentm)):
                ...
                parent, encode, decode = rae(children)
                loss = criterion(decode, children)
                optimizer.zero_grad()
